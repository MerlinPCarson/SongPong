...
D = 40 * 40 # input dimensionality: 80x80 grid
with cp.cuda.Device(0):
    if resume:
      ...
      model = {}
      model['W1'] = np.random.randn(D,H) / np.sqrt(H) # "PrELU" initialization
...
while episode_number < num_episodes:
  ...
  if done: # an episode finished
    ...
    learning_rate = start_lrate + (episode_number * 1.0 / num_episodes) * (end_lrate - start_lrate) 
    print 'resetting env. episode reward total was %f. running mean: %f, new Learning Rate: %f' % (reward_sum, running_reward, learning_rate)
